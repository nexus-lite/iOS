...
- (void) viewWillAppear:(BOOL)animated
{
    ...
    [[STATRManager sharedManager] logFPS:2 viewController:self];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    STATREvent *yourCustomEvent = [[STATREvent alloc] initWithName:@"Radio button"
                                                                   date:[NSDate date]
                                                         viewController:self];
    // Add parameter to custom event
    [yourCustomEvent addLogParameterWithName:@"Product license changed"
                                       value:[[NSUserDefaults standardUserDefaults] objectForKey:@"product_package"]];
