...
#import "StatrIOS-CELogVC.h"

@interface StatrIOS_CELogVC ()

@end

@implementation StatrIOS_CELogVC
...
- (void) viewDidAppear:(BOOL)animated
{
    ...
    [[STATRManager sharedManager] logFPS:7 viewController:self];
    [[STATRManager sharedManager] logFPS:22 viewController:self];
    [[STATRManager sharedManager] logFPS:45 viewController:self];
}
/**********************************************************************************************************
                            Button handler : "AddCustomLogEventButtonHandler"
 *********************************************************************************************************/
- (IBAction)AddCustomLogEventButtonHandler:(UIButton *)sender forEvent:(UIEvent *)event
{
    // Initialization of custom event
    STATREvent *yourCustomLogEvent = [[STATREvent alloc] initWithName:@"Sent Button"
                                                                 date:[NSDate date]
                                                                event:event
                                                       viewController:self];
    // Add log parameter to custom event
    [yourCustomLogEvent addLogParameterWithName:self.textEditWithName.text
                                          value:self.textEditWithValue.text];
    // Add your custom event to STATR tracker
    [[STATRManager sharedManager] addCustomEvent:yourCustomLogEvent];
}
/**********************************************************************************************************
