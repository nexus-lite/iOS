//
//  StatrIOS-CEStringVC.m
//  StatrIOSTestProject
//
//  Copyright (c) 2013 STATR. All rights reserved.
//

#import "StatrIOS-CEStringVC.h"

@interface StatrIOS_CEStringVC()

@end

@implementation StatrIOS_CEStringVC
...
- (void) viewDidAppear:(BOOL)animated
{
    ...
    [[STATRManager sharedManager] logFPS:20 viewController:self];
    [[STATRManager sharedManager] logFPS:22 viewController:self];
}
/**********************************************************************************************************
                            Button handler : "AddCustomStringEventButtonHandler"
 *********************************************************************************************************/
- (IBAction)AddCustomStringEventButtonHandler:(UIButton *)sender forEvent:(UIEvent *)event
{
    // Initialization of custom event
    STATREvent *yourCustomStringEvent = [[STATREvent alloc] initWithName:@"Sent Button"
                                                                    date:[NSDate date]
                                                                   event:event
                                                          viewController:self];
    // Add string parameter to custom event
    [yourCustomStringEvent addTextParameterWithName:self.textEditWithName.text
